...
public class IncidenciasService {
	...
	public Double[] getDataCircle() {
		...
		for (EstadoTipos eT : EstadoTipos.values()) {
			ret[aux] = calculatePercentage(eT);
			aux++;
		}
		return ret;
	}

	private Double calculatePercentage(EstadoTipos state) {
		/* Mejorar cuando sepamos hacer querys en mongo... */
		...
		return (values.size() / inci.size()) * 100.0;
	}
	...
	public void recogerNuevasIncidencias() {
		 ...
		 */
		Incidencia inci1 = new Incidencia("Inci" + (numeroIncis + 1), "descripcion" + (numeroIncis + 1),
				"Lat" + (numeroIncis + 1), "Lon" + (numeroIncis + 1), Choy.getTime(), CunaSemana.getTime(), idAgente1,
				propiedades1, categorias1);

		/*
		 * PROPIEDADES = PRESION, VELOCIDAD_CIRCULACION CATEGORIAS = FUEGO, INUNDACION
		 * ESTADO = ABIERTA - SIN OPERARIO
		 */
		Incidencia inci2 = new Incidencia("Inci" + (numeroIncis + 2), "descripcion" + (numeroIncis + 2),
				"Lat" + (numeroIncis + 2), "Lon" + (numeroIncis + 2), Choy.getTime(), CunaSemana.getTime(), idAgente2,
				propiedades2, categorias2);
