...
public class InsertSampleDataService {
	...
	public void init() {
		/*
		 * Aquí se crean los objetos y y luego se llama al service oportuno para
		 * añadirlo al repositorio, en el service solo debe haber un metodo que llame al
		 * save de repository, que ya esta por Crud
		 */
		// Creación de agentes
		...
		Incidencia inci1 = new Incidencia("Inci1", "descripcion1", "Lat1", "Lon1", Choy.getTime(), CunaSemana.getTime(),
				agente1, propiedades1, categorias1);
		incidenciaService.addIncidencia(inci1);

		/*
		 * PROPIEDADES = PRESION, VELOCIDAD_CIRCULACION CATEGORIAS = FUEGO, INUNDACION
		 * ESTADO = ABIERTA - SIN OPERARIO
		 */
		Incidencia inci2 = new Incidencia("Inci2", "descripcion2", "Lat2", "Lon2", Choy.getTime(), CunaSemana.getTime(),
				agente1, propiedades2, categorias2);
		incidenciaService.addIncidencia(inci2);

		/*
		 * PROPIEDADES = VELOCIDAD_VIENTO CATEGORIAS = METEOROLOGICA ESTADO = EN PROCESO
		 */
		Incidencia inci3 = new Incidencia("Inci3", "descripcion3", "Lat3", "Lon3", Choy.getTime(), CunaSemana.getTime(),
				agente1, propiedades3, categorias3);
		inci3.asignarOperario(op1);
		incidenciaService.addIncidencia(inci3);

		/*
		 * PROPIEDADES = VELOCIDAD_VIENTO CATEGORIAS = VALOR_NO_ASIGNADO ESTADO = EN
		 * PROCESO
		 */
		Incidencia inci4 = new Incidencia("Inci4", "descripcion4", "Lat4", "Lon4", Choy.getTime(), CunaSemana.getTime(),
				agente4, propiedades3, categorias4);
		inci3.asignarOperario(op2);
		incidenciaService.addIncidencia(inci4);

		/*
		 * PROPIEDADES = VALOR_NO_ASIGNADO CATEGORIAS = METEOROLOGICA ESTADO = EN
		 * PROCESO
		 */
		Incidencia inci5 = new Incidencia("Inci5", "descripcion5", "Lat5", "Lon5", Choy.getTime(), CunaSemana.getTime(),
				agente4, propiedades4, categorias3);
		inci3.asignarOperario(op3);
		incidenciaService.addIncidencia(inci5);

		/*
		 * PROPIEDADES = VALOR_NO_ASIGNADO CATEGORIAS = VALOR_NO_ASIGNADO ESTADO = EN
		 * PROCESO
		 */
		Incidencia inci6 = new Incidencia("Inci5", "descripcion", "Lat5", "Lon5", Choy.getTime(), CunaSemana.getTime(),
				agente4, propiedades4, categorias4);
		inci3.asignarOperario(op3);
		incidenciaService.addIncidencia(inci6);

		/*
		 * PROPIEDADES = TEMPERATURA, HUMEDAD CATEGORIAS = FUEGO, INUNDACION ESTADO = EN
		 * PROCESO
		 */
		Incidencia inci7 = new Incidencia("Inci6", "descripcion6", "Lat6", "Lon6", Choy.getTime(), CunaSemana.getTime(),
				agente1, propiedades1, categorias2);
		inci7.asignarOperario(op1);
		inci7.cerrarIncidencia();
		incidenciaService.addIncidencia(inci7);

		/*
		 * PROPIEDADES = PRESION, VELOCIDAD_CIRCULACION CATEGORIAS = ACCIDENTE_AEREO,
		 * ACCIDENTE_CARRETERA ESTADO = ANULADA
		 */
		Incidencia inci8 = new Incidencia("Inci7", "descripcion1", "Lat7", "Lon7", Choy.getTime(), CunaSemana.getTime(),
				agente1, propiedades2, categorias1);
